        ...
        List.for_all
          (fun (color, regex) ->
            ...
            |> List.for_all (fun item ->
                   let v =
                     String.split_on_char ' ' item |> List.hd |> int_of_string
                   in
                   let max = Hashtbl.find check_for_games_with color in
                   v <= max))
          (Hashtbl.to_seq regexes |> List.of_seq)
      in
...
let parse_line (line : string) = String.split_on_char ':' line |> game

let _compute (lines : string list) =
  List.map parse_line lines |> List.fold_left ( + ) 0
